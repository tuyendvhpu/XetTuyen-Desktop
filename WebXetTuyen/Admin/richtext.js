...
function writeRichText(rte, html, width, height, buttons, readOnly) {
	if (isRichText) 
	{
		...
		if (isIE) {
			if (buttons && (width < 350)) width = 350;
...
function dlgInsertTable(rte, command) {
	...
	parent.command = command;
	currentRTE = rte;
	var windowOptions = 'history=no,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=no,resizable=yes,width=360,height=200';
